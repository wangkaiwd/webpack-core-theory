...
class AutoExternalPlugin {
  ...
  apply (compiler) {
    compiler.hooks.normalModuleFactory.tap(this.name, (normalModuleFactory) => {
      this.getImportedPackage(normalModuleFactory);
      this.createGlobalVariable(normalModuleFactory);
    });
    this.injectScriptsToHTML(compiler);
  }

  getImportedPackage (normalModuleFactory) {
  ...
  createGlobalVariable (normalModuleFactory) {
    // AsyncSeriesBailHook
